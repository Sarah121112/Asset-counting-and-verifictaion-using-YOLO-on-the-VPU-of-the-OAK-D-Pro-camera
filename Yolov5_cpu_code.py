...
# === YOLOv5 Model (used only for region proposals) ===
model = YOLO("yolov5l.pt")
 
# === Reference image matching using ORB ===
...
ax1.axis('off')
ax1.set_title("One-Shot Bottle Detection (ORB)")
...
def send_bottle_stats():
    global total_match_count, avg_confidence, frame_times, fps_values
    while True:
        ...
        temp = psutil.sensors_temperatures().get('cpu-thermal', [{}])[0].get('current', 0.0)
        cpu_usage = psutil.cpu_percent(interval=None)
        voltage = 5.0
        power_watts = (cpu_usage / 100) * voltage * psutil.cpu_count(logical=False) * 1000
...
def update_frame(_):
    global total_match_count, avg_confidence, frame_times, fps_values, confidence_scores, power_values
    start = time.time()
